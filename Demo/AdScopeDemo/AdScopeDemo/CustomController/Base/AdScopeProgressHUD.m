...
#define AdScopeMainThreadAssert() NSAssert([NSThread isMainThread], @"AdScopeProgressHUD needs to be accessed on the main thread.");

CGFloat const AdScopeProgressMaxOffset = 1000000.f;

static const CGFloat AdScopeDefaultPadding = 4.f;
static const CGFloat AdScopeDefaultLabelFontSize = 16.f;
static const CGFloat AdScopeDefaultDetailsLabelFontSize = 12.f;


@interface AdScopeProgressHUD ()
...
@interface AdScopeProgressHUDRoundedButton : UIButton
@end


@implementation AdScopeProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    AdScopeProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    AdScopeProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (AdScopeProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            AdScopeProgressHUD *hud = (AdScopeProgressHUD *)subview;
...
- (void)commonInit {
    _animationType = AdScopeProgressHUDAnimationFade;
    _mode = AdScopeProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(AdScopeProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    if (type == AdScopeProgressHUDAnimationZoom) {
        type = animatingIn ? AdScopeProgressHUDAnimationZoomIn : AdScopeProgressHUDAnimationZoomOut;
    }

    CGAffineTransform small = CGAffineTransformMakeScale(0.5f, 0.5f);
    CGAffineTransform large = CGAffineTransformMakeScale(1.5f, 1.5f);
    UIView *bezelView = self.bezelView;
    if (animatingIn && bezelView.alpha == 0.f && type == AdScopeProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == AdScopeProgressHUDAnimationZoomOut) {
        bezelView.transform = large;
    }
    dispatch_block_t animations = ^{
        if (animatingIn) {
            bezelView.transform = CGAffineTransformIdentity;
        } else if (!animatingIn && type == AdScopeProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == AdScopeProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    AdScopeProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<AdScopeProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    AdScopeBackgroundView *backgroundView = [[AdScopeBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = AdScopeProgressHUDBackgroundStyleSolidColor;
    ...
    AdScopeBackgroundView *bezelView = [AdScopeBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:AdScopeDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:AdScopeDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [AdScopeProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:AdScopeDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[AdScopeRoundProgressView class]];

    AdScopeProgressHUDMode mode = self.mode;
    if (mode == AdScopeProgressHUDModeIndeterminate) {
    ...
    else if (mode == AdScopeProgressHUDModeDeterminateHorizontalBar) {
        [indicator removeFromSuperview];
        indicator = [[AdScopeBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == AdScopeProgressHUDModeDeterminate || mode == AdScopeProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            [indicator removeFromSuperview];
            indicator = [[AdScopeRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == AdScopeProgressHUDModeAnnularDeterminate) {
            [(AdScopeRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == AdScopeProgressHUDModeCustomView && self.customView != indicator) {
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == AdScopeProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[AdScopeProgressHUD class], nil];
#else
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[AdScopeProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[AdScopeRoundProgressView class]]) {
        AdScopeRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [AdScopeRoundProgressView appearanceWhenContainedIn:[AdScopeProgressHUD class], nil];
#else
        appearance = [AdScopeRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[AdScopeProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((AdScopeRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((AdScopeRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[AdScopeBarProgressView class]]) {
        AdScopeBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [AdScopeBarProgressView appearanceWhenContainedIn:[AdScopeProgressHUD class], nil];
#else
        appearance = [AdScopeBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[AdScopeProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((AdScopeBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((AdScopeBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    AdScopeBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? AdScopeDefaultPadding : 0.f;
...
- (void)setMode:(AdScopeProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == AdScopeProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation AdScopeRoundProgressView
...
@implementation AdScopeBarProgressView
...
@interface AdScopeBackgroundView ()
...
@implementation AdScopeBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = AdScopeProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(AdScopeProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    AdScopeProgressHUDBackgroundStyle style = self.style;
    if (style == AdScopeProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == AdScopeProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation AdScopeProgressHUDRoundedButton
