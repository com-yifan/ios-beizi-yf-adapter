//
//  AMPSSplashViewController.m
//  AdScopeDemo
//
//  Created by Cookie on 2023/6/9.
//

#import "AMPSSplashViewController.h"
...
@interface AMPSSplashViewController () <UITextFieldDelegate, AMPSSplashAdDelegate>
...
@implementation AMPSSplashViewController
...
- (void)ampsSplashAdLoadSuccess:(AMPSSplashAd *)splashAd {
    ...
    CookieAMPSLog(@"成功渠道：%@，价格：%ld,id:%@", self.splash.successAdInfo.adapterClassName, (long)[self.splash eCPM],splashAd.successAdInfo.adapterSpaceId);
...
- (void)viewDidLoad {
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:kAdScopeDemoAMPSSDKSplashID]) {
        self.spaceId = [[NSUserDefaults standardUserDefaults] objectForKey:kAdScopeDemoAMPSSDKSplashID];
...
- (AMPSSplashAd *)splash {
    if (!_splash) {
        ...
        UIView *logoView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 120)];
        logoView.backgroundColor = [UIColor orangeColor];
        cfg.bottomView = logoView;
        _splash = [[AMPSSplashAd alloc]initWithSpaceId:self.spaceId adConfiguration:cfg];
